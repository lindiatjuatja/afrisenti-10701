...
class SentClassProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, lang):
        """See base class."""
        file = os.path.join(data_dir, f"{lang}_train.csv")
    ...
    def get_dev_examples(self, data_dir, lang):
        ...
        # file = glob(file)[0]
        # sents = self.read_json_file(file)
        file = os.path.join(data_dir, f"{lang}_dev.csv")
        sents = pd.read_csv(file)
    ...
    def _create_examples(self, lines, set_type):
        examples = []
        for i, (sentence, label) in lines.iterrows():
            guid = "%s-%s" % (set_type, i)
            text_a = sentence
            text_b = None
...
def read_data(data_dir, processor, tokenizer, lang, split, max_seq_length, model_name, bert_model_type="ori", train_size=-1, seed=42):
    ...
    if os.path.isfile(pt_name):
        ...
        label_ids = torch.tensor([example.label for example in examples], dtype=torch.long)
        # if this does not work, try using this label for each token in the input ids ?
...
def main():
    ...
    parser.add_argument("--bert_model", default='xlm-roberta-base',
                        type=str,
                        #required=True,
    ...
    parser.add_argument('--source_language_strategy', type=str, default="specified", choices=["random", "language_family", "specified", "random2"],
                        help='The strategy to select source languages.')
    parser.add_argument('--portion', type=int, default=2,
                        help="1/n used for training")
    parser.add_argument('--source_languages', type=str, default="en",
                ...
                for j, train_s_loader in enumerate(train_s_loaders): # note here
                    ...
                    for step, batch_train_s in enumerate(tqdm(train_s_loader, desc="Iteration")): # count epoch based on merged src langs loader
                        ...
                        if args.method == "metaxl":
                            ...
                            if half == 0:
                                train_t_ids, train_t_mask, train_t_labels = eval_ids, eval_mask, eval_labels
